...
export class ClaudeProvider implements AgentProvider {
  ...
  private lastRateLimitResetsAt: number | null = null;

  public readonly metadata: AgentProviderMetadata = {
    name: 'claude',
  ...
  getRateLimitResetsAt(): number | null {
    return this.lastRateLimitResetsAt;
  }

  clearRateLimitResetsAt(): void {
    this.lastRateLimitResetsAt = null;
  }
   ...
   */
  async generatePlan(prompt: string, options: ProviderOptions = {}, onOutput?: (text: string) => void): Promise<PlanResult> {
          ...
          try {
            ...
            if (type === 'assistant') {
              ...
              if (info?.resetsAt != null) {
                this.lastRateLimitResetsAt = info.resetsAt as number;
              }
            }
          } catch {
            // Not valid JSON, skip
  ...
  private parseStreamEvent(output: ClaudeJsonOutput): StreamEvent | null {
    ...
    switch (type) {
      ...
      case 'rate_limit_event': {
        ...
        if (!info) return null;
        if (info.resetsAt != null) {
          this.lastRateLimitResetsAt = info.resetsAt;
        }
        return {
          type: 'rate_limit',
          rateLimitInfo: {
